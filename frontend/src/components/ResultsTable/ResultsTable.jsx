...
function TableIcon() {
  return (
    <svg className="results-table-icon" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
...
function StatusPill({ status }) {
  ...
  const { cls, label } = map[status] || { cls: 'pill-pending', label: status || 'pending' };
...
function PlaylistCell({ url: rawUrl, type = 'tv', status = 'success' }) {
  const [copied, setCopied] = useState(false);
  const url = cleanM3uUrl(rawUrl);
  ...
  if (!url) {
    if (type === 'vod') {
      return (
        <span className="playlist-none-badge vod-none" title="No VOD playlist provided by this service">
      ...
      <div className="results-table-scroll">
        {results.length === 0 ? (
          <div className="results-empty-state">
            ...
            <div className="results-empty-title">No playlists harvested yet</div>
            <p className="results-empty-desc">
              Select your email provider and target services, then click <strong>Start Automation</strong> to capture Live TV and VOD playlists automatically.
            </p>
          </div>
        ) : filteredResults.length === 0 ? (
          <div className="results-empty-state">
            <div className="results-empty-title">No results matching "{searchQuery}"</div>
            <p className="results-empty-desc">Try searching for a different keyword or clear the search field.</p>
          </div>
        ) : viewMode === 'table' ? (
          /* ── Table View ─────────────────────────────────────────────── */
            ...
            <tbody>
              {filteredResults.map((r, i) => (
                <tr key={i}>
                  ...
                  <td>
                    {r.duration ? (
                      <div className="duration-expiry-stack">
                        ...
                        {r.expiresAt && (
                          <span className="expiry-time-text" title={`Exact Expiration: ${r.expiresAt}`}>
                            📅 {r.expiresAt}
                          </span>
                        )}
                      </div>
                  ...
                  <td><PlaylistCell url={r.tvPlaylist} type="tv" /></td>
                  <td><PlaylistCell url={r.vodPlaylist} type="vod" /></td>
              ...
              <div key={i} className="playlist-card-item">
                ...
                <div className="card-m3u-box">
                  <div className="card-m3u-row">
                    ...
                    <span className="card-m3u-link-preview" title={r.tvPlaylist}>
                      {cleanM3uUrl(r.tvPlaylist) || 'Waiting for confirmation email…'}
                    </span>
                    {r.tvPlaylist && <PlaylistCell url={r.tvPlaylist} type="tv" />}
                  </div>
                  ...
                  <div className="card-m3u-row">
                    ...
                    {r.vodPlaylist ? (
                      <>
                        <span className="card-m3u-link-preview" title={r.vodPlaylist}>
                          {cleanM3uUrl(r.vodPlaylist)}
                        </span>
                        <PlaylistCell url={r.vodPlaylist} type="vod" status={r.status} />
                      </>
                    ) : (
                      <span className="playlist-none-badge vod-none" title="No VOD playlist provided by this service">
                        <span className="none-symbol">🚫</span>
